...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_netbox_plugin_ext_cluster_xxx
namespace = Collection("netbox_plugin_ext_cluster")
namespace.configure(
    {
        "netbox_plugin_ext_cluster": {
            "netbox_ver": "3.5.9",
            "python_ver": "3.10",
            "project_name": "netbox_plugin_ext_clusters",
            "local": False,
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.netbox_plugin_ext_cluster.compose_http_timeout,
        "NETBOX_VER": context.netbox_plugin_ext_cluster.netbox_ver,
        "PYTHON_VER": context.netbox_plugin_ext_cluster.python_ver,
    }
    compose_command = f'docker-compose --project-name {context.netbox_plugin_ext_cluster.project_name} --project-directory "{context.netbox_plugin_ext_cluster.compose_dir}"'
    for compose_file in context.netbox_plugin_ext_cluster.compose_files:
        compose_file_path = os.path.join(context.netbox_plugin_ext_cluster.compose_dir, compose_file)
...
def run_command(context, command, **kwargs):
    ...
    if is_truthy(context.netbox_plugin_ext_cluster.local):
        context.run(command, **kwargs)
    else:
        # Check if netbox is running, no need to start another netbox container to run a command
        docker_compose_status = "ps --services --filter status=running"
        results = docker_compose(context, docker_compose_status, hide="out")
        if "ext-cluster-ui" in results.stdout:
            compose_command = f"exec ext-cluster-ui {command}"
        else:
            compose_command = f"run --entrypoint '{command}' ext-cluster-ui"
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Netbox with Python {context.netbox_plugin_ext_cluster.python_ver}...")
